if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Marco22's Script is already running!", 0)
    return
end
...
Title.TextSize = 18
Title.Text = "Marco22's Script FE v" .. currentVersion
...
PluginsHint.Text = "Download plugins from the IY Discord (discord.gg/2P996dUjPT)"
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://discord.gg/2P996dUjPT")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.gg/2P996dUjPT"
	end)
	dragGUI(main)
	main.Parent = ScaledHolder
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Marco22's Script Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Marco22's Script support server.'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://discord.gg/2P996dUjPT')
		notify('Discord Invite', 'Copied to clipboard!\ndiscord.gg/2P996dUjPT')
	else
		notify('Discord Invite', 'discord.gg/2P996dUjPT')
	end
	if httprequest then
		httprequest({
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Marco22's Script may have issues or features that are not functioning correctly.") end
end)